...
class Countries extends Component {
  ...
  addCountry = id => {
    this.setState(prevState => ({
      countriesList: prevState.countriesList.map(eachList => {
        if (eachList.id === id) {
          return {...eachList, isVisited: true}
        }
        return eachList
      }),
    }))
  ...
  removeCountry = id => {
    this.setState(prevState => ({
      countriesList: prevState.countriesList.map(eachList => {
        if (eachList.id === id) {
          return {...eachList, isVisited: false}
        }
        return eachList
      }),
    }))
  ...
  render() {
    ...
    return (
      <div className="main-container">
        ...
        {updateList.length === 0 ? (
          <p className="no-view">No Countries Visited Yet!</p>
        ) : (
